#!/usr/bin/env bash
# run_tag.sh — Nur taggen: Rohlinge aus work/streaming_results/ anreichern + einsortieren
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

CONFIG="${CONFIG:-config_tagging.json}"
...
uv run radio-ripper tag --config "$CONFIG" &
PID=$!
echo "$PID" > "$PID_FILE"
wait "$PID"
RC=$?
